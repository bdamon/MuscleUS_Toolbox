...
%    and the third dimension includes row and column (Y and X) positions. The 
...
%% Get variations from input structurefunction [fiber_all_pixels, stop_list] = fiber_track_us(vector_image, roi_struc, image_data_struc, ft_options, fv_options)
%
%FUNCTION fiber_track_us
%  [fiber_all_pixels, stop_list] = fiber_track_us(vector_image, roi_struc, image_data_struc, ft_options, fv_options);
%
...
if isfield(ft_options, 'show_image')
    show_image = ft_options.show_image;
else 
    show_image = 0;
end
...
% Get seed points
roi_points(1,:) = roi_struc.fitted_roi_r_pixels;                                     %column number
