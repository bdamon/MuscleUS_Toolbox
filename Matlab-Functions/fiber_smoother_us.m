...
%
%  image_gray, roi_struc, fv_options (optional): If included, the existence of 
%    these variables will prompt fiber_visualizer_us to be automatically 
%    called so that the smoothed tracts can be visualized.
%
%OUTPUT ARGUMENTS
...
%%
coeff_c_pixels = zeros(length(fiber_all_pixels(:,1,2)), poly_order(1)+1);
coeff_r_pixels = zeros(length(fiber_all_pixels(:,1,1)), poly_order(2)+1);
coeff_x_mm = zeros(length(fiber_all_pixels(:,1,2)), poly_order(1)+1);
coeff_y_mm = zeros(length(fiber_all_pixels(:,1,1)), poly_order(2)+1);

smoothed_fiber_all_pixels = zeros(size(fiber_all_pixels));
smoothed_fiber_all_mm = smoothed_fiber_all_pixels;
for track_cntr=1:length(fiber_all_pixels(:,1,1))

    if length(nonzeros(fiber_all_pixels(track_cntr,:,2)))>5

        loop_track_points = [nonzeros(fiber_all_pixels(track_cntr,:,1)) nonzeros(fiber_all_pixels(track_cntr,:,2))];
