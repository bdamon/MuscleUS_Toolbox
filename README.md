...
* Downloading code and data: The easiest way to do this is to click once on the green Code button. Select the "Download ZIP" option and extract the files to a convenient place on your computer (note that after extracting the files, you will need to set a MATLAB path to the directory that holds the custom-written functions). The file named "M9NG1Q0C" is a DICOM-formatted ultrasound image file suitable for testing out the code. 
